...
from f1tenth_gym_jax.envs.collision_models import collision, collision_map, get_vertices
...
class CollisionTests(unittest.TestCase):
    def setUp(self):
        # set up track
        self.env = make(
            "Spielberg_3_noscan_collision_progress_acceleration+steeringvelocity_1_5_v0"
        )
        ...
        self.all_poses = jnp.tile(self.test_pose, (32, 1))
        rng = jax.random.key(0)
        noises = jax.random.normal(rng, shape=(32, 3)) / 10
        ...
        perturbations = jnp.linspace(-2.5, 2.5, 25)
        self.map_test_poses = jnp.stack(
            jnp.meshgrid(perturbations, perturbations), axis=-1
        ).reshape(-1, 2)
        self.map_test_poses = jnp.hstack(
            (self.map_test_poses, jnp.zeros((self.map_test_poses.shape[0], 1)))
        )
